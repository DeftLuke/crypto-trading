...
/** Demo/paper trading — unlimited trades and relaxed daily loss caps. */
function isDemoTradingMode() {
  const controlMode = (process.env.CONTROL_TRADING_MODE || 'demo').toLowerCase();
  if (controlMode === 'live' && config.binance.tradingMode === 'live' && !config.binance.demo && !config.binance.testnet) {
    return false;
  }
  return (
    controlMode === 'demo'
    || config.binance.tradingMode === 'demo'
    || config.binance.demo
    || config.binance.testnet
  );
}

function hasUnlimitedDemoRisk(signal = {}) {
  if (signal.demo_mode === true || signal.demo_unlimited === true) return true;
  if (isDemoTestMode(signal)) return true;
  if (isDemoTradingMode()) return true;
  return false;
}

function isUnlimitedDemoTest(signal = {}) {
  if (hasUnlimitedDemoRisk(signal)) return true;
...
async function validateScannerTradeExecution(signal) {
  ...
  if (!hasUnlimitedDemoRisk(signal) && todayCount >= config.strategy.maxDailyTrades) {
    passed = false;
    checks.push({
      rule: 'max_trades',
      passed: false,
    ...
    checks.push({
      rule: 'max_trades',
      passed: true,
      message: hasUnlimitedDemoRisk(signal)
        ? `Demo mode — unlimited (${todayCount} today)`
  ...
  if (!hasUnlimitedDemoRisk(signal) && dailyPnl < 0 && Math.abs(dailyPnl) >= maxLoss) {
    passed = false;
    checks.push({
      rule: 'daily_loss',
      passed: false,
...
export async function validateTelegramTradeExecution(signal) {
  ...
  const dailyPnl = await getTodayDailyPnl();
  const maxLoss = balance * config.strategy.maxDailyLoss;
  if (!unlimitedDemo && dailyPnl < 0 && Math.abs(dailyPnl) >= maxLoss) {
    passed = false;
    checks.push({
      rule: 'daily_loss',
      passed: false,
