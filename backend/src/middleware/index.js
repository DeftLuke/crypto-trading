...
╚══════════════════════════════════════════════╝
  ...
  if (config.telegram.token && config.telegram.pollingEnabled) {
    await setupTelegramPolling(handleTelegramCallback);
    await sendStartupNotification();
  } else if (config.telegram.delivery === 'n8n') {
    await sendStartupNotification();
    console.log('[Telegram] Backend polling disabled; n8n owns Telegram commands and notifications');
  }
});
